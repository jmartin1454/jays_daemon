...
def update_status(todays_things):
    todays_things.__init__()  # reset everything to False
    now = datetime.datetime.now(pytz.timezone('America/New_York'))
    day = mlbgame.day(now.year, now.month, now.day, home=myteam, away=myteam)
    if(len(day)==0):
        return False
    game = day[0]
    todays_things.game_today = True

    status = game.game_status
    todays_things.status=status

    todays_things.score_home = game.home_team_runs
    todays_things.score_away = game.away_team_runs

    if (status == 'IN_PROGRESS'):
        todays_things.game_on = True
    elif (status == 'FINAL'):
        todays_things.game_over = True
    elif (status == 'PRE_GAME'):
        todays_things.pre_game = True
    ...
    todays_things.minutes_to_go=minutes_to_go
    ...
    todays_things.score_string = game.nice_score()

    if(status=='FINAL'):
        todays_things.game_won = (athome and (todays_things.score_home>todays_things.score_away) or (not athome and (todays_things.score_home<todays_things.score_away)))
